...
import { PostAuthRequest, GetAuthRequest } from "../../helper/authRequest";
...
const PostsPage = () => {
  ...
  useEffect(() => {
    const successFunction = (response) => {
      ...
      const commentSuccessFunction = (res) => {
        console.log(res)
        setCommentArray(res.data.comments);
      };
      GetAuthRequest("api/comment/fetch?postId=" + response.data.post.postId, commentSuccessFunction, enqueueSnackbar, navigate, setLoading);
    };
    GetAuthRequest("api/post/get?slug=" + slug, successFunction, enqueueSnackbar, navigate, setLoading);
  }, []);
          ...
          <div className={styles.container}>
            ...
            <div className={styles.right_container}>
              <div className={styles.header}>
                <Avatar className={styles.profilePicture} src={post.profileImage} alt="image" onClick={() => navigate("/" + post.userName)} />
                <div className={styles.post_username} onClick={() => navigate("/" + post.userName)}>
                  {post.userName}
                </div>
              </div>
                  ...
                  commentArray
                    ...
                    .map((e, i) => {
                      return (
                        <Comment
                          key={e._id}
                          userName={e.commentor.userName}
                          text={e.text}
                          time={e.createdAt}
                          profileImage={e.commentor.profileImageUrl}
                        />
                      );
                    })
                ) : (
                  <div className={styles.noCommentDiv}>
                    <h1 className={styles.heading_no_comment}>No comment yet</h1>
                    <p className={styles.text_no_comment}>Start the conversation</p>
                  </div>
                )}
              </div>
              <div>
                ...
                <div className={styles.post_like}>{likeCount} likes</div>

                <div className={styles.postCommentCount}>{commentCount > 0 ? "View all " + commentCount + " comments" : "Be the first to comment..."}</div>

                <div className={styles.timeCount}>
                  <h1 className={styles.time}>{time(parseInt(Date.now() / 1000) - parseInt(post.createdAt))}</h1>
                </div>
                <form className={styles.comment_container} onSubmit={postComment}>
                  <input className={styles.commentbox} type="text" placeholder="Add a comment..." ref={commentBox_large} onChange={(e) => setComment(e.target.value)} required/>
                  {comment != "" ? (
                    <button
      ...
      <div className={`${styles.main_container} ${styles.small_screen}`}>
        <div className={styles.main}>
          ...
          <div className={styles.icons_container}>
            ...
            <form className={styles.comment_container} onSubmit={postComment}>
              <input className={styles.commentbox} type="text" placeholder="Add a comment..." ref={commentBox} onChange={(e) => setComment(e.target.value)} required />
              {comment != "" ? (
                <button
                  className={styles.post_comment}
                  onClick={(e) => {
          ...
          <div className={styles.comments}>
            {commentArray && commentArray.length > 0 ? (
              commentArray
                ...
                .reverse()
                .map((e, i) => {
                  return <Comment key={e.id} userName={e.commentorUserName} text={e.text} time={e.createdAt} profileImage={e.profileImage} />;
                })
            ) : (
              <div className={styles.noCommentDiv}>
                <h1 className={styles.heading_no_comment}>No comment yet</h1>
